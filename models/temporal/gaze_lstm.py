...
class GazeLSTMClassifier:
    ...
    def train(self, X, y, continue_training: bool = False):
        ...
        print(
            f"Initialized LSTM classifier ({phase}, seq_length={self.seq_length}, hidden={self.hidden_size}, layers={self.num_layers})"
        )
        ...
        if len(X_seq) == 0:
            if continue_training:
                print(
                    f"⚠️ Fine-tune skipped: no sequences (seq_length={self.seq_length})."
                )
                return
            raise ValueError(
    ...
    def predict(self, X):
        ...
        self.model.eval()
        with torch.no_grad():
            X_tensor = torch.from_numpy(np.asarray(X_seq, dtype=np.float32)).to(
                self.device
            )
